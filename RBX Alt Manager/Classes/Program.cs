            ...
            if (mutex.WaitOne(TimeSpan.Zero, true))
            {
                ...
                try
                {
                    ...
                    using (Apply773Fix ? new FileStream(CookiesFile, FileMode.Open, FileAccess.ReadWrite, FileShare.None) : null)
                    {
                        ...
                        // Capturar exceções não tratadas na thread UI.
                        // ObjectDisposedException de controles dinâmicos é suprimido
                        // (causado por mensagens de mouse pendentes após RefreshItemsPanel).
                        Application.SetUnhandledExceptionMode(UnhandledExceptionMode.CatchException);
                        Application.ThreadException += (sender, args) =>
                        {
                            if (args.Exception is ObjectDisposedException)
                            {
                                Logger.Warn($"ObjectDisposedException suprimido: {args.Exception.Message}");
                                return;
                            }
                            
                            MessageBox.Show(
                                args.Exception.ToString(),
                                "Blox Brasil Gerenciador de Contas",
