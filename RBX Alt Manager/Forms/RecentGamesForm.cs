...
namespace RBX_Alt_Manager.Forms
{
    public partial class RecentGamesForm : Form
    {
        ...
        public event EventHandler<GameArgs> RecentGameDeleted;
        ...
        private void AddGameControl(Game Game)
        {
            ...
            RControl.DeleteRequested += RControl_DeleteRequested;
        ...
        private void RControl_DeleteRequested(object sender, GameArgs e)
        {
            // Remover o controle da UI
            var control = sender as GameControl;
            if (control != null)
            {
            ...
            // Notificar para remover da lista de jogos recentes
            RecentGameDeleted?.Invoke(this, new GameArgs(e.Game));
        }

        /// <summary>
        /// Remove um jogo específico da lista visual
        /// </summary>
        public void RemoveGame(Game game)
        {
            GameControl controlToRemove = null;
            foreach (Control ctrl in GamesPanel.Controls)
            {
                if (ctrl is GameControl gc && gc.Game?.Details?.placeId == game.Details?.placeId)
                {
                    controlToRemove = gc;
                    break;
                }
            ...
            {
                GamesPanel.Controls.Remove(controlToRemove);
                controlToRemove.Dispose();
            }
        }

        /// <summary>
        /// Recarrega toda a lista de jogos
        /// </summary>
        public void RefreshGames()
        {
            GamesPanel.Controls.Clear();
            for (int i = AccountManager.RecentGames.Count - 1; i >= 0; i--)
                AddGameControl(AccountManager.RecentGames[i]);
        }
